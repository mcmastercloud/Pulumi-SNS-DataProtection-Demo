...
import { Response } from "./classes.js";
...
export async function createTopic(attributes) {
    ...
    if (updated_attributes?.kmsMasterKeyId == undefined) {
        ...
        const kms_key = new aws.kms.Key(`kms-${attributes.name}`, {
            policy: policy.json,
            description: `kms-sns-${attributes.name}`,
            tags: { "name": `kms-sns-${attributes.name}` }
        });
        new aws.kms.Alias(`kms-sns-alias-${attributes.name}`, {
            name: `alias/sns/${attributes.name}`,
            targetKeyId: kms_key.id
        });
        updated_attributes.kmsMasterKeyId = kms_key.id;
    }
    const sns_topic = new aws.sns.Topic(attributes.name, updated_attributes);
...
function addDataProtectionPolicy(aws_account, aws_region, topic_name, dataProtectionIdentifiers) {
    ...
    const create_trigger = dataProtectionIdentifiers.join();
